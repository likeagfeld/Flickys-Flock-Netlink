 ...
 *
 * Adapted from Disasteroids name_entry.c for Flicky's Flock.
...
static char g_name_buf[FNET_MAX_NAME + 1];
...
void nameEntry_init(void)
{
    ...
    if (g_Game.playerName[0] == '\0') {
        if (jo_backup_mount(JoInternalMemoryBackup)) {
            if (jo_backup_file_exists(JoInternalMemoryBackup, "FLOCK_NAME")) {
                unsigned int blen = 0;
                void* data = jo_backup_load_file_contents(
                    JoInternalMemoryBackup, "FLOCK_NAME", &blen);
                if (data && blen > 0 && blen <= FNET_MAX_NAME + 1) {
                    memcpy(g_Game.playerName, data, blen);
                    g_Game.playerName[FNET_MAX_NAME] = '\0';
                    g_bkp_load_status = 1;
                } else {
                    g_bkp_load_status = 4;
                }
    ...
    if (g_Game.playerName[0] != '\0') {
        int i;
        g_name_len = 0;
        for (i = 0; g_Game.playerName[i] && i < FNET_MAX_NAME; i++) {
...
void nameEntry_input(void)
{
    ...
    if (g_Game.gameState != GAMESTATE_NAME_ENTRY) return;
        ...
        if (g_Game.input.pressedABC == false) {
            if (g_cursor_row == 4 && g_cursor_col == ROW4_DEL) {
                ...
                if (g_name_len > 0) {
                    ...
                    {
                        static char bkp_fname[] = "FLOCK_NAME";
                ...
                if (ch != 0 && g_name_len < FNET_MAX_NAME) {
...
void nameEntry_draw(void)
{
    ...
    font_printf(FONT_X(6), FONT_Y(20), 500, "      %d/%d", g_name_len, FNET_MAX_NAME);
