 ...
 * using VDP1 sprites instead of VDP2 jo_printf text.
 */
...
void font_load(void)
{
    ...
    g_fontBase = jo_sprite_add_tga_tileset(NULL, "FONT.TGA",
...
void font_draw(const char* str, int x, int y, int z)
{
    ...
    while (*str) {
        ...
        if (c >= FONT_FIRST && c <= FONT_LAST) {
            /* jo_sprite_draw3D draws from center; add half-char offset */
